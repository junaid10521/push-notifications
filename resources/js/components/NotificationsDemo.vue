<template>
  <div>
    ...
    <!-- Send notification button type - 2 -->
    <button
      :disabled="loading"
      type="button"
      class="btn btn-success btn-send" @click="sendNotificationType2"
    >
      Send Notification Type 2
    </button>
...
export default {
  ...
  methods: {
    ...
    sendNotificationType2 () {
      this.loading = true

      axios.post('/notifications-type-2')
        .catch(error => console.log(error))
        .then(() => { this.loading = false })
    },
